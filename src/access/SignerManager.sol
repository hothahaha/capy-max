...
contract SignerManager is UUPSUpgradeableBase {
    ...
    function initialize(address initialSigner, uint256 _threshold) external initializer {
    ...
    function updateThreshold(uint256 _threshold) external onlyMultiSig {
        if (_threshold == 0 || _threshold > signerCount) revert SignerManager__InvalidThreshold();

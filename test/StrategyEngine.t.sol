...
contract StrategyEngineTest is Test {
  ...
  function setUp() public {
    ...
    (engine, cpToken, vault, signerManager, multiSig, helperConfig) = deployer
      .run();
  ...
  function test_WithdrawWBTC() public withWBTCDeposit {
    ...
    // 验证状态更新
    (uint256 newTotalWbtc, , uint256 newTotalBorrows, ) = engine.getUserTotals(
      user
    );
  ...
  function test_RevertWhen_WithdrawWBTCWithHighBorrow() public withWBTCDeposit {
    ...
    // 尝试提取小于借贷金额的 USDC
    uint256 invalidWithdrawAmount = engineBalance + (engineBalance * 10) / 100; // 减少 100 USDC
